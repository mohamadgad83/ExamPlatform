...
export function formatDate(date: string | Date, options?: { 
  ...
  if (options?.relative) {
    return formatDistanceToNow(d, { addSuffix: true, locale: ar } as any);
  }

  if (options?.withTime) {
    return format(d, "dd MMMM yyyy - HH:mm", { locale: ar as any });
  }

  return format(d, "dd MMMM yyyy", { locale: ar as any });
}
